...
### テスト方針
- **ローカルサーバーは起動しない** - このプロジェクトでは `npm run dev` 等でサーバーを起動しない
- 変更は全てプッシュして、GitHub Pages上で動作確認を行う
- 開発版（`-dev`）やデバッグモードはこのワークフローのために用意されている
...
- **CLAUDE.mdはコミット・プッシュしない** - このファイルはローカル開発用ドキュメント
