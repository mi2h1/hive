...
|--------------|--------------------------------------------------------------------------|
| **要素**     | **実装方針**                                                             |
| カメラ       | PerspectiveCamera `[0,6,5]` FOV=30, target=`[0,0,0.4]`（自家河寄り）   |
| ライティング | Environment studio (intensity=0.1) + AmbientLight(0.4) + DirectionalLight×2 (key=0.5, fill=0.2) |
| 卓           | PlaneGeometry + フェルト風テクスチャ（放射グラデーション+微細ノイズ）     |
| 卓枠         | RoundedBoxGeometry（角丸）+ Canvas生成の木目テクスチャ                   |
| 中央パネル   | 黒い正方形（1.2×1.2）。局数・ドラ・残牌数の表示用                       |
| ドラ牌       | 表向きで少し傾けて配置（未実装）                                         |
...
- MeshPhysicalMaterial を使用

- 側面4面: ガラス風光沢（clearcoat=0.8, clearcoatRoughness=0.05, roughness=0.15, ior=1.5）。表面側 7/9 はアイボリー(#f5f0e8)、背面側 2/9 はブラウン(#cc9900)で色分け

- 絵柄面(+Z): マット仕上げ（clearcoat=0, roughness=0.8）+ bumpMap で彫り込み表現（bumpScale=-0.025）

- 背面(-Z): ブラウン(#cc9900) 単色、控えめな光沢（clearcoat=0.6, roughness=0.25）

- テクスチャに anisotropy=16 を設定（斜め視点でのぼやけ防止）
...
|--------------|------------------------------------------------------------------------------------------------------------|------------------------------|
| **フェーズ** | **内容**                                                                                                   | **目標**                     |
| Phase 1      | 牌の3Dモデル表示 + 卓3D表現 ・RoundedBoxGeometry + MeshPhysicalMaterial ・画像テクスチャで絵柄描画 ・bumpMapで彫り込み表現 ・フェルト風テーブル + 木目枠 ・4家の牌配置 ・ライティング調整 | **完了** 卓上に牌が並んだ状態  |
