...
const cleanupOldRooms = async () => {
  try {
    const roomsRef = ref(db, 'spark-rooms');
    ...
    for (const [code, room] of Object.entries(rooms)) {
      ...
      if (now - createdAt > maxAge || playerCount === 0) {
        deletePromises.push(remove(ref(db, `spark-rooms/${code}`)));
      }
    }

    if (deletePromises.length > 0) {
      await Promise.all(deletePromises);
      console.log(`Cleaned up ${deletePromises.length} old spark rooms`);
    }
  } catch (err) {
...
export const useRoom = (playerId: string | null, playerName: string | null) => {
  ...
  useEffect(() => {
    ...
    const roomRef = ref(db, `spark-rooms/${roomCode}`);
    const myPresenceRef = ref(db, `spark-rooms/${roomCode}/presence/${playerId}`);
  ...
  useEffect(() => {
    ...
    const presenceListRef = ref(db, `spark-rooms/${roomCode}/presence`);
    const roomRef = ref(db, `spark-rooms/${roomCode}`);
    ...
    onValue(roomRef, (snapshot) => {
      ...
      if (data) {
        ...
        const normalizedPlayers = normalizeArray<Player>(gs?.players).map(p => ({
          ...p,
          vault: normalizeArray<Gem>(p.vault),
          secured: normalizeArray<Gem>(p.secured),
        }));

        // プラットフォームの宝石配列も正規化
        const normalizedPlatforms = normalizeArray<Platform>(gs?.platforms).map(p => ({
          ...p,
  ...
  const createRoom = useCallback(async () => {
    ...
    try {
      ...
      const roomRef = ref(db, `spark-rooms/${code}`);
  ...
  const joinRoom = useCallback(async (code: string) => {
    ...
    try {
      ...
      const roomRef = ref(db, `spark-rooms/${upperCode}`);
      ...
      return new Promise<boolean>((resolve) => {
        onValue(roomRef, async (snapshot) => {
          ...
          await update(ref(db, `spark-rooms/${upperCode}/gameState`), {
  ...
  const leaveRoom = useCallback(async () => {
    ...
    try {
      ...
      if (isHost) {
        await remove(ref(db, `spark-rooms/${roomCode}`));
      } else {
        ...
        if (updatedPlayers.length === 0) {
          ...
          await update(ref(db, `spark-rooms/${roomCode}/gameState`), {
            players: updatedPlayers,
          });
        }
      }
  ...
  const updateGameState = useCallback(async (newState: Partial<GameState>) => {
    ...
    try {
      await update(ref(db, `spark-rooms/${roomCode}/gameState`), newState);
    } catch (err) {
  ...
  // プレイヤーのアクションを設定
  const setPlayerAction = useCallback(async (action: PlayerAction) => {
    if (!roomCode || !playerId || !roomData) return;

    const players = roomData.gameState.players;
    const updatedPlayers = players.map(p => {
      if (p.id === playerId) {
        return { ...p, action, isReady: true };
      }
      return p;
    });
